...
-- Update the evaluate_tree_growth function with corrected RAISE syntax
...
BEGIN
    ...
    LOOP
        ...
        -- Select a random event (guaranteed to get one)
        -- First try weighted random based on rarity
        ...
        -- If no event was selected, pick any random event (guaranteed fallback)
        ...
        event_water_change := COALESCE(random_event.water_modifier, 0);
        event_sunlight_change := COALESCE(random_event.sunlight_modifier, 0);
        event_feed_change := COALESCE(random_event.feed_modifier, 0);
        event_love_change := COALESCE(random_event.love_modifier, 0);
        
        RAISE NOTICE 'Selected event: % (water: %, sunlight: %, feed: %, love: %)', 
            random_event.name, event_water_change, event_sunlight_change, event_feed_change, event_love_change;
        ...
        RAISE NOTICE 'Care counts after event - Water: %, Sunlight: %, Feed: %, Love: %',
            care_counts.water_count, care_counts.sunlight_count, 
            care_counts.feed_count, care_counts.love_count;
